...
\KwData{String value $s$}
\KwResult{Data Type index of $s$}
\If{$s$ can be parsed as $int$}{
  \State \Return 1
}
\If{$s$ can be parsed as $long$}{
  \State \Return 2
}
\If{$s$ can be parsed as $double$}{
  \State \Return 3
}
\If{$s$ can be parsed as $date$}{
  \State \Return 4
}
\State \Return 0
\end{algorithm}
